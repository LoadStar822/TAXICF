...
namespace taxicf {
	class InterleavedCuckooFilter {
		...
		static inline uint32_t fast_rand_u32() {
			...
			if (state == 0) {
				...
				seed ^= reinterpret_cast<uint64_t>(&state);
				state = seed ? seed : 0x106689d45497fdb5ULL;
			}
			return static_cast<uint32_t>(splitmix64(state));
		}
	...
	public:
		...
		bool kickOut(size_t binIndex, size_t value, uint16_t tag)
		{
			if (bitNum == 16)
			{
				...
				for (int count = 0; count < MaxCuckooCount; count++)
				{
					...
					size_t randPos = static_cast<size_t>(fast_rand_u32()) % TagNum;
		...
		* -----------------------------------------------------------------------------------------------
		*/
		inline void bulk_contain(size_t value, std::vector<uint8_t>& result)
		{
			assert(result.size() >= bins);
			if (bitNum == 16)
			{
			...
			else if (bitNum == 8)
			{
				...
				for (size_t batch = 0; batch < bin_words; batch++)
				{
					...
					result[batch * 2 + 0] = match0 ? 1 : 0;
					result[batch * 2 + 1] = match1 ? 1 : 0;
				}
			}
		}
		...
		*/
		...
		inline std::vector<int> bulk_count(value_range_t&& values)
		{
			std::vector<int> result(bins, 0);
			std::vector<uint8_t> tmp_result(bins, 0);

			for (auto value : values) {
				std::fill(tmp_result.begin(), tmp_result.end(), 0);
				bulk_contain(value, tmp_result);
				for (size_t i = 0; i < bins; ++i) {
					result[i] += static_cast<int>(tmp_result[i]);
				}
			}
