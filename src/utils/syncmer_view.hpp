...
public:
    ...
    */
    basic_iterator(urng1_iterator_t urng1_iterator,
        ...
        positions{positions}
    {
        // NOTE: `std::ranges::distance(it, sentinel)` can be O(n) for non-sized views and would
        // introduce an extra full pass over the sequence just to validate the window size.
        // We only need to ensure the first range has at least `window_size` elements.
        auto it_check = this->urng1_iterator;
        for (size_t i = 0; i < window_size; ++i)
        {
            if (it_check == this->urng1_sentinel)
            {
